...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Đăng ký sự kiện tìm kiếm cuộc hội thoại
...
function renderConversations() {
    ...
    if (typeof contactSearchQuery !== 'undefined' && contactSearchQuery) {
        filtered = conversations.filter(conv => {
            return (conv.name && conv.name.toLowerCase().includes(contactSearchQuery)) ||
                   (conv.username && conv.username.toLowerCase().includes(contactSearchQuery));
        });
    }

    if (!filtered.length) {
        list.innerHTML = `<p class="text-center text-gray-400 text-xs py-8">Không tìm thấy cuộc hội thoại nào.</p>`;
        return;
    }
...
function updateConversationMsg(idOrUsername, isGroup, msg) {
    ...
    if (conv) {
        ...
        } else if (msg.fileUrl && (!msg.content || msg.content.trim() === '')) {
...
async function loadFriends() {
    try {
        const friends = await apiGet(`${API_URL}/chat/contacts`);
        
        let groups = [];
        try {
            groups = await apiGet(`${API_URL}/chat/groups`);
        } catch (err) {
...
async function selectChat(username, fullName) {
    ...
    const user = checkAuth();
    try {
        const data = await apiGet(`${API_URL}/chat/history?user2=${username}`);
...
async function selectGroupChat(groupId, groupName) {
    ...
    try {
        const data = await apiGet(`${API_URL}/chat/group-history?conversationId=${groupId}`);
        const history = data.messages || [];
    ...
    try {
        ...
        // Tải avatar nhóm bằng hàm dùng chung
        updateGroupAvatarUI(groupId, group.avatar, group.name);
        
        // If sidebar is visible, reload it too
        const sidebar = document.getElementById('groupInfoSidebar');
        if (sidebar && !sidebar.classList.contains('hidden')) {
            loadGroupInfoSidebar(group);
        }
    } catch (e) {
...
function appendMessage(message, type) {
    ...
    if (message.isRevoked) {
        ...
        if (message.fileUrl) {
            ...
            if (isImage) {
                mediaHtml = `<img src="${message.fileUrl}" class="rounded-lg mb-2 max-w-full block" style="max-height: 200px; object-fit: cover;">`;
            } else {
                const fileName = message.fileName || message.fileUrl.split('/').pop().substring(37);
                const fileInfo = getFileExtInfo(fileName, message.fileUrl);
                mediaHtml = `
                    <a href="${message.fileUrl}" download="${escapeHtml(fileName)}" class="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-xl hover:bg-gray-100 transition mb-2 no-underline text-gray-800 max-w-xs lg:max-w-md">
                        <div class="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-xs flex-shrink-0 ${fileInfo.bgClass}">
                            ${fileInfo.extText}
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="text-xs font-semibold truncate text-gray-900">${escapeHtml(fileName)}</div>
                            <div class="text-[10px] text-gray-400">Nhấn để tải xuống</div>
                        </div>
                        <div class="text-gray-400 hover:text-primary transition p-1">
...
async function recallMessage(messageId) {
    ...
    try {
        await apiPut(`${API_URL}/chat/revoke/${messageId}`);
...
async function clearHistory() {
    ...
    try {
        if (isGroup) {
            await apiDelete(`${API_URL}/chat/group-history/${currentConversationId}`);
            document.getElementById('chatMessages').innerHTML = '';
            selectGroupChat(currentConversationId, targetName);
        } else {
            await apiDelete(`${API_URL}/chat/history/${currentChatUser}`);
            document.getElementById('chatMessages').innerHTML = '';
...
function setupMessageForm() {
    ...
    form.addEventListener('submit', async e => {
        ...
        if (imageFile) {
             ...
             try {
                  const uploadData = await apiPost(`${API_URL}/upload/chat-image`, formData);
                  if (uploadData.status === 'ok') {
                      fileUrl = uploadData.fileUrl || uploadData.imageUrl;
                      fileName = uploadData.fileName || imageFile.name;
                  }
             } catch (err) {
...
async function submitCreateGroup() {
    ...
    const category = document.getElementById("groupCategoryInput").value;
    ...
    try {
        await apiPost(`${API_URL}/chat/group`, { name, usernames, privacy, category, description });
        closeGroupModal(); 
        document.getElementById("groupNameInput").value = "";
        document.getElementById("groupPrivacyInput").value = "PRIVATE";
        document.getElementById("groupCategoryInput").value = "Khác";
...
async function leaveGroup() {
    ...
    try {
        await apiPost(`${API_URL}/chat/group/${currentConversationId}/leave`);
...
async function loadFriendsForAddSelection() {
    ...
    try {
        const friends = await apiGet(`${API_URL}/chat/group/${currentConversationId}/non-members`);
        if (friends.length === 0) {
...
async function submitAddMember() {
    ...
    try {
        await apiPost(`${API_URL}/chat/group/${currentConversationId}/add-members`, { usernames });
        closeAddMemberModal();
...
async function loadGroupInfoSidebar(groupData = null) {
    if (!currentConversationId) return;
    loadGroupMembersTab();
    try {
        // Sử dụng dữ liệu truyền vào nếu có, tránh gọi API lần 2
        const group = groupData ? groupData : await apiGet(`/api/group/${currentConversationId}`);
        ...
        // Cập nhật Avatar bằng hàm dùng chung
        updateGroupAvatarUI(currentConversationId, group.avatar, group.name);
...
async function disbandGroup() {
    ...
    try {
        await apiDelete(`${API_URL}/group/${currentConversationId}`);
        ...
        loadFriends(); // Refresh sidebar list
    } catch (err) { alert(err.message || 'Lỗi khi giải tán nhóm'); }
}
...
async function deleteDocument(docId) {
    ...
    try {
        await apiDelete(`${API_URL}/document/${docId}`);
        alert('Đã xóa tài liệu thành công!');
        loadGroupDocumentsTab();
    } catch (e) {
        console.error(e);
        alert(e.message || 'Lỗi khi xóa tài liệu');
    }
}

async function togglePinDocument(docId) {
    try {
        await apiPost(`${API_URL}/document/${docId}/pin`);
        loadGroupDocumentsTab();
    } catch (e) {
        console.error(e);
        alert(e.message || 'Lỗi khi ghim tài liệu');
    }
}
...
async function submitUploadDoc() {
    ...
    try {
        await apiPost(`${API_URL}/document/upload`, formData);
        closeUploadDocModal();
        fileInput.value = '';
...
async function saveRenameGroup() {
    ...
    try {
        ...
        // Cập nhật tên nhóm mới lên các nhãn text
        document.getElementById('sidebarGroupName').textContent = res.name;
        document.getElementById('chatTitle').textContent = res.name;
        
        // Cập nhật trong mảng cục bộ conversations và lấy avatar hiện tại
        const conv = conversations.find(c => c.isGroup && c.id == currentConversationId);
        let existingAvatar = null;
        if (conv) {
            conv.name = res.name;
            existingAvatar = conv.avatar;
        }
        
        // Gọi hàm dùng chung để đồng bộ avatar mới (kể cả avatar mặc định dạng chữ)
        updateGroupAvatarUI(currentConversationId, existingAvatar, res.name);
        
        // Cập nhật tên hiển thị ở danh sách phòng chat bên trái
...
async function uploadGroupAvatar() {
    ...
    try {
        const data = await apiPost(`${API_URL}/group/${currentConversationId}/avatar`, formData);
...
async function openGroupSettingsModal() {
    ...
    try {
        ...
        // 1. Điền dữ liệu vào chế độ Đọc
        const privacyMap = { 'PRIVATE': 'Riêng tư', 'PUBLIC': 'Công khai', 'REQUIRES_APPROVAL': 'Yêu cầu duyệt' };
        document.getElementById('viewGroupPrivacy').textContent = privacyMap[group.privacy] || group.privacy;
        document.getElementById('viewGroupCategory').textContent = group.category || 'Khác';
        document.getElementById('viewGroupDesc').textContent = group.description || 'Không có mô tả';
        
        // 2. Điền dữ liệu vào Form Chỉnh sửa
        document.getElementById('editGroupPrivacy').value = group.privacy;
        document.getElementById('editGroupCategory').value = group.category || 'Khác';
        document.getElementById('editGroupDesc').value = group.description || '';
        
        // 3. Phân quyền hiển thị theo vai trò thành viên
        const role = window.currentUserGroupRole;
        const viewModeEl = document.getElementById('settingsViewMode');
        const editModeEl = document.getElementById('settingsEditMode');
        ...
        const disbandBtn = document.getElementById('btnDisbandGroupInline');
        
        if (role === 'ADMIN' || role === 'CO_ADMIN') {
            // Hiển thị chế độ Chỉnh sửa
            viewModeEl.classList.add('hidden');
            editModeEl.classList.remove('hidden');
            titleEl.textContent = "Cài đặt nhóm";
            saveBtn.classList.remove('hidden');
            cancelBtn.textContent = "Hủy";
            
            // Chỉ hiển thị nút Giải tán nhóm cho Trưởng nhóm (ADMIN)
            if (disbandBtn) {
                disbandBtn.classList.toggle('hidden', role !== 'ADMIN');
            }
        } else {
            // Thành viên thường: Chỉ hiển thị chế độ xem Chỉ đọc
            viewModeEl.classList.remove('hidden');
            editModeEl.classList.add('hidden');
            titleEl.textContent = "Thông tin cấu hình nhóm";
        ...
        console.error("Lỗi mở modal cài đặt nhóm:", err);
    }
}
...
async function saveGroupSettings() {
    ...
    try {
        const res = await apiPost(`/api/group/${currentConversationId}/settings`, { privacy, category, description });
        alert("Cập nhật cấu hình nhóm thành công!");
        closeGroupSettingsModal();
        
        // Tải lại sidebar để hiển thị dữ liệu mới đồng bộ
        loadGroupInfoSidebar();
    } catch (err) {
        alert(err.message || "Lỗi khi cập nhật cài đặt nhóm.");
    }
}