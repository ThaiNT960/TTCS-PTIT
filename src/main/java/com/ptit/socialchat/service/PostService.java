...
import com.ptit.socialchat.repository.CommentRepository;
import com.ptit.socialchat.repository.PostLikeRepository;
...
public class PostService {
    ...
    @Autowired
    private com.ptit.socialchat.repository.ModerationSettingsRepository moderationSettingsRepository;
    @Autowired
    ...
    public Map<String, Object> getAllPosts(User currentUser, String search, int page, int size) {
        ...
        if (search != null && !search.trim().isEmpty()) {
            postsPage = postRepository.findByStatusAndContentContainingIgnoreCaseOrderByCreatedAtDesc("APPROVED",
                    search.trim(), pageable);
        } else {
            postsPage = postRepository.findByStatusOrderByCreatedAtDesc("APPROVED", pageable);
        }
    ...
    public List<PostDTO> getAllPostsForAdmin(User currentUser, String search) {
        List<Post> posts;
        if (search != null && !search.trim().isEmpty()) {
            posts = postRepository.searchPostsAdmin(search.trim());
        } else {
    ...
    public Map<String, Object> getPostsByUser(User targetUser, User viewerUser, int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<Post> postsPage = postRepository.findByUserAndStatusOrderByCreatedAtDesc(targetUser, "APPROVED", pageable);
        List<PostDTO> content = postsPage.getContent().stream()
    ...
    public Comment addComment(Long postId, String content, User user, Long parentId) {
        ...
        notificationService.createNotification(post.getUser(), user, com.ptit.socialchat.enums.NotificationType.COMMENT, "home.html#post-" + postId);
        return savedComment;
    }
    ...
    public boolean reactToPost(Long postId, User user, String reactionType) {
        ...
        if (isNewReaction && !post.getUser().getId().equals(user.getId())) {
            notificationService.createNotification(post.getUser(), user, com.ptit.socialchat.enums.NotificationType.LIKE, "home.html#post-" + postId);
        }
    ...
    public boolean reactToComment(Long commentId, User user, String reactionType) {
        ...
        Optional<com.ptit.socialchat.entity.CommentReaction> existing = commentReactionRepository.findByCommentAndUser(comment, user);
        ...
        if (existing.isPresent()) {
            com.ptit.socialchat.entity.CommentReaction reaction = existing.get();
            if (reactionType.equals(reaction.getReactionType().name())) {
                commentReactionRepository.delete(reaction);
                return false;
            } else {
                reaction.setReactionType(com.ptit.socialchat.enums.ReactionType.valueOf(reactionType));
                commentReactionRepository.save(reaction);
                isNewReaction = true;
            }
        } else {
            com.ptit.socialchat.entity.CommentReaction reaction = new com.ptit.socialchat.entity.CommentReaction();
        ...
        if (isNewReaction && !comment.getUser().getId().equals(user.getId())) {
            notificationService.createNotification(comment.getUser(), user, com.ptit.socialchat.enums.NotificationType.LIKE, "home.html#post-" + comment.getPost().getId());
        }
        return isNewReaction;
    }
        ...
        dto.setComments(post.getComments().stream()
            ...
            .map(c -> {
                ...
                if (c.getReactions() != null) {
                    ...
                    if (currentUser != null) {
                        Optional<com.ptit.socialchat.entity.CommentReaction> currentCReac = c.getReactions().stream()
